...
Use when the user says `/api-design-review` or asks to review, audit, or check an OpenAPI spec against standards. Do not use to generate a spec (that is `design-api-spec`) or to review implementation code.
...
2. If `RMOA_API_KEY` is available, re-lint with RateMyOpenAPI (see the `design-api-spec` skill, Step 5) and confirm the score did not regress.
